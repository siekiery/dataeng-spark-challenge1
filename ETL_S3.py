...
def create_spark_session():
    """Starts a spark session running locally with connection to AWS
...
def save_to_s3(df, bucket_name, output_name):
    """Writes enriched dataframe to S3 bucket in csv and snappy.parquet formats
    Args:
    ...
    Returns:
        None
    """
    output_name_csv = f's3a://{bucket_name}/{output_name}.csv'
    output_name_parquet = f's3a://{bucket_name}/{output_name}.snappy.parquet'
...
def main():
    """Main ETL body. Starts spark session and runs ETL procedures. Stores enriched data on S3 bucket.
    """
    input_filepath = 'data.csv'
    abbr_filepath = 'state_abbreviations.csv'

    bucket_name = 'xxx'  # placeholder for AWS S3 bucketname
    ...
    # Writes enriched data to AWS S3 bucket
    print(f"> Writing enriched data to AWS S3 {bucket_name} bucket")
    save_to_s3(df, bucket_name, output_name)
